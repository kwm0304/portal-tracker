...
async function readAndProcessFile() {
  try {
    ...
    for (const entry of data) {
      const school = entry.newSchool.toLowerCase().replace(/\s+/g, '-');
      if (school === '') {
        continue;
      }
    ...
    const result = Object.keys(schools).map(school => ({ school, players: schools[school] }));
...
async function main() {
  try {
    const result = await readAndProcessFile();
    const urls = getUrls(result, sport, year);
    console.log(urls);
  } catch (error) {
...
function getUrls(result, sport, year) {
  let ref = '';
  if (sport === 'ncaab') {
    ref = 'cbb';
  } else if (sport === 'ncaaf') {
  ...
  for (const school of result) {
    const url = `https://www.sports-reference.com/${ref}/schools/${school.school}/${year}.html`;
    urls.push(url);
  }
...
// async function scrapePlayer(browser, sport, school, year) {
//   let ref = ''
//   if (sport === 'ncaab') {
//     ref = 'cbb'
//   } else if (sport === 'ncaaf') {
...
//   const url = `https://www.sports-reference.com/${ref}/schools/${school}/men/${year}.html`;
